...
router.put('/user/:username', auth.verificaAcesso, function(req, res){
...
router.post('/',auth.verificaAcesso,function(req, res) {
  ...
  userModel.register(new userModel({ name: req.body.name,
...
router.post('/login', function(req, res, next) {
  passport.authenticate('local', function(err, user) {
    ...
    req.logIn(user, function(err) {
      ...
      jwt.sign(
        { username: req.user.username, level: req.user.level, sub: 'aula de EngWeb2024' },
        'EngWeb2024',
        { expiresIn: 3600 },
        function(e, token) {
          if (e) {
...
router.delete('/user/:username',auth.verificaAcesso,function(req,res){
